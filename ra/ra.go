...
func (ra *RegistrationAuthorityImpl) issueCertificateOuter(
	...
	cert, profileHash, err := ra.issueCertificateInner(
	...
	if err != nil {
		...
		logEvent.CertProfileName = order.CertificateProfileName
...
func (ra *RegistrationAuthorityImpl) issueCertificateInner(
	...
	profileName string,
	acctID accountID,
	oID orderID) (*x509.Certificate, []byte, error) {
	if features.Get().AsyncFinalize {
	...
	return parsedCertificate, precert.CertProfileHash, nil
}
