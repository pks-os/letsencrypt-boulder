...
func TestIssueCertificateInnerWithProfile(t *testing.T) {
	...
	_, profileHash, err := ra.issueCertificateInner(context.Background(), csr, "default", 1, 1)
...
func TestIssueCertificateOuter(t *testing.T) {
	...
	_, err = ra.issueCertificateOuter(context.Background(), order, csr, logEvent)
	test.AssertNotError(t, err, "Could not issue certificate")
	test.AssertMetricWithLabelsEquals(t, ra.newCertCounter, prometheus.Labels{"profileName": order.CertificateProfileName, "profileHash": fmt.Sprintf("%x", mockCA.profileHash)}, 1)
}
